#include "libraries/pico_synth_i2s/pico_synth_i2s.hpp"
...
mp_obj_t Channel_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    mp_raise_msg(&mp_type_RuntimeError, "Cannot create Channel objects. They can only be accessed from PicoSynth.synth_channel()");
...
/***** Variables Struct *****/
typedef struct _PicoSynth_obj_t {
    mp_obj_base_t base;
    PicoSynth_I2S* picosynth;
} _PicoSynth_obj_t;

/***** Print *****/
void PicoSynth_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind) {
    (void)kind; //Unused input parameter
    //_PicoSynth_obj_t *self = MP_OBJ_TO_PTR2(self_in, _PicoSynth_obj_t);
    mp_print_str(print, "PicoSynth()");
}


/***** Constructor *****/
mp_obj_t PicoSynth_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    _PicoSynth_obj_t *self = nullptr;

    enum { ARG_i2s_data, ARG_i2s_bclk, ARG_i2s_lrclk, ARG_pio, ARG_sm };
    static const mp_arg_t allowed_args[] = {
...
/***** Destructor ******/
mp_obj_t PicoSynth___del__(mp_obj_t self_in) {
    _PicoSynth_obj_t *self = MP_OBJ_TO_PTR2(self_in, _PicoSynth_obj_t);
    m_del_class(PicoSynth_I2S, self->picosynth);
    return mp_const_none;
}
...
extern mp_obj_t PicoSynth_set_volume(mp_obj_t self_in, mp_obj_t value) {
    _PicoSynth_obj_t *self = MP_OBJ_TO_PTR2(self_in, _PicoSynth_obj_t);
    self->picosynth->set_volume(mp_obj_get_float(value));
    return mp_const_none;
}

extern mp_obj_t PicoSynth_get_volume(mp_obj_t self_in) {
    _PicoSynth_obj_t *self = MP_OBJ_TO_PTR2(self_in, _PicoSynth_obj_t);
    return mp_obj_new_float(self->picosynth->get_volume());
}

extern mp_obj_t PicoSynth_adjust_volume(mp_obj_t self_in, mp_obj_t delta) {
    _PicoSynth_obj_t *self = MP_OBJ_TO_PTR2(self_in, _PicoSynth_obj_t);
    self->picosynth->adjust_volume(mp_obj_get_float(delta));
    return mp_const_none;
}
...
extern mp_obj_t PicoSynth_play_sample(mp_obj_t self_in, mp_obj_t data) {
    ...
    self->picosynth->play_sample((uint8_t *)bufinfo.buf, bufinfo.len);
